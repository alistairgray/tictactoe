...
$('.box').on('click', function(event){
  ...
  if(gameStatus === 'won' || gameStatus === 'draw'){
    ...
    alert('Box already clicked, pick another');
      ...
      // Adds an image of 'O' to clicked box
      ...
      // Changes status div to display new text and red background indicating Player X's turn
      $('#start').text("Player X's Turn").css({'font-size':'30pt','background-color': 'red'});
      gameChecker(playerOGame);

      // Changes the currentPlayer variable to Player X
      currentPlayer = 'playerX'
